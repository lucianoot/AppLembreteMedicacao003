...
public partial class MainPage : ContentPage
{
    ...
    private async Task GerarCicloAutomatico(int medId, int intervalo)
    {
        ...
        // Quantidade de horários dentro de 24h
        ...
        for (int i = 0; i < repeticoes; i++)
        {
           ...
           var novoCronograma = new Cronograma
            {
                MedicamentoId = medId,
                Hora = dataDose.ToString("HH:mm"),
                Frequencia = $"{intervalo}h/{intervalo}h",
                Ativo = 1
            };
        ...
        // Salva todas as doses
        await App.Banco.InsertDoses(listaDosesParaBanco);

        await DisplayAlert(
            "Sucesso",
            $"Ciclo de {intervalo}h criado e horários adicionados!",
            "OK"
        );
